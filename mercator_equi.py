...
mercator = cv2.imread('custom.png', cv2.IMREAD_UNCHANGED)

# Convert the Mercator image to RGBA if it is not already in RGBA format
if mercator.shape[2] == 3:
    mercator = cv2.cvtColor(mercator, cv2.COLOR_BGR2RGBA)

# Define the dimensions of the Equirectangular projection
equi_height = 2048
mercator_aspect = mercator.shape[1] / mercator.shape[0]
equi_width = int(equi_height * mercator_aspect * 2)
